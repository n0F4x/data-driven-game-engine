...
export module modules.scheduler.accessors.events.Recorder;
...
export template <modules::events::event_c... Events_T>
...
public:
    ...
    constexpr explicit Recorder(
        modules::events::BufferedEventQueue<Events_T>&... buffered_event_queues
    );
...
private:
    std::tuple<std::reference_wrapper<modules::events::BufferedEventQueue<Events_T>>...>
...
template <modules::events::event_c... Events_T>
    requires(sizeof...(Events_T) != 0)
constexpr modules::scheduler::accessors::events::Recorder<Events_T...>::Recorder(
    modules::events::BufferedEventQueue<Events_T>&... buffered_event_queues
)
...
template <typename... Args_T>
    requires(sizeof...(Events_T) == 1)
         ...
         && std::constructible_from<
                util::meta::type_list_front_t<util::TypeList<Events_T...>>,
                Args_T&&...>
constexpr auto modules::scheduler::accessors::events::Recorder<Events_T...>::record(
    Args_T&&... args
) const -> void
{
