...
export module ddge.modules.execution.v2.gatherers.gatherer_builder_of_c;
...
concept gatherer_builder_of_c =
    requires { typename T::Result; }
    && requires(
        T&&                                        gatherer_builder,
        const uint32_t                             number_of_gathers,
        TaskFinishedCallback<typename T::Result>&& callback
    ) {
           {
               std::move(gatherer_builder).build(number_of_gathers, std::move(callback))
           } -> shared_gatherer<Input_T>;
       };
