...
export module ddge.modules.exec.locks.LockGroup;
...
constexpr auto ddge::exec::LockGroup::expand(Lock&& lock) -> void
{
    expand(util::meta::hash<T>(), std::move(lock));
}

constexpr auto ddge::exec::LockGroup::expand(const LockGroup& lock_group) -> void
{
...
auto ddge::exec::LockGroup::transform_top_of_lock_stack(
    TopOfLockStack&&     top_of_lock_stack,
...
constexpr auto
    ddge::exec::LockGroup::expand(const LockedResourceID resource_id, Lock&& lock) -> void
{
