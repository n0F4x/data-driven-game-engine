...
export module ddge.modules.exec.gatherers.AllOf;
...
ddge::exec::AllOf::Precondition::Precondition(const uint32_t capacity)
{
    PRECOND(capacity > 0);
}

ddge::exec::AllOf::AllOf(const uint32_t capacity)
...
auto ddge::exec::AllOf::set_continuation(TaskContinuation<bool>&& continuation) -> void
{
    m_continuation = std::move(continuation);
}

auto ddge::exec::AllOf::receive(const bool value) -> void
{
    if (m_capacity == 1) {
        call_callback(value);
    }
...
auto ddge::exec::AllOf::call_callback(const bool output) -> void
{
