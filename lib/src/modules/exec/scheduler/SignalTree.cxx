...
auto ddge::exec::SignalTree::try_set_one(const LeafIndex leaf_index) -> bool
{
...
auto ddge::exec::SignalTree::try_unset_one(const TravelsalTactic tactic)
...
auto ddge::exec::SignalTree::number_of_leaves() const noexcept -> uint32_t
{
    return 0x1u << (number_of_levels() - 1);
}

auto ddge::exec::SignalTree::number_of_branches() const noexcept -> uint32_t
{
    return number_of_nodes() - number_of_leaves();
}

auto ddge::exec::SignalTree::number_of_nodes() const noexcept -> uint32_t
{
    return static_cast<uint32_t>(m_nodes.size());
}

ddge::exec::SignalTree::Precondition::Precondition(const uint32_t number_of_levels)
{
