...
module ddge.modules.exec.TaskHubBuilder;

import ddge.modules.exec.ErasedTaskFactory;
import ddge.modules.exec.TaskHubProxy;

ddge::exec::TaskHubBuilder::TaskHubBuilder(Nexus& nexus) : m_nexus{ nexus } {}

auto ddge::exec::TaskHubBuilder::locks_of(const TaskIndex task_index) const
    -> const LockGroup&
{
    return get(task_index).locks();
}

auto ddge::exec::TaskHubBuilder::build(
