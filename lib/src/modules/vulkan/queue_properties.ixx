...
export [[nodiscard]]
auto first_present_queue_family_index(
    const vk::raii::PhysicalDevice& physical_device,
...
{
    for (const uint32_t family_index : std::views::iota(
             0u, static_cast<uint32_t>(physical_device.getQueueFamilyProperties2().size())
         ))
    {
