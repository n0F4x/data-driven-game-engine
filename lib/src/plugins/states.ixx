...
export class States {
public:
    template <modules::states::state_c State_T, app::decays_to_builder_c Self_T>
...
) && -> app::add_on_t<App_T, addons::States>
{
    return std::forward<App_T>(app).add_on(
        addons::States{
            .state_manager = modules::states::StateManager{ std::move(m_states) } }
    );
}
