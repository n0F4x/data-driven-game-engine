module;

#include <functional>

export module ddge.util.meta.algorithms.fold_left_first;

import ddge.util.meta.algorithms.apply;
import ddge.util.meta.concepts.integer_sequence.index_sequence;
import ddge.util.meta.concepts.type_list.type_list;
import ddge.util.meta.type_traits.integer_sequence.integer_sequence_size;
import ddge.util.meta.type_traits.type_list.type_list_at;
import ddge.util.meta.type_traits.type_list.type_list_size;

namespace ddge::util::meta {

export template <index_sequence_c IndexSequence_T, typename Transform_T, typename Operation_T>
    requires(integer_sequence_size_v<IndexSequence_T> != 0)
constexpr auto fold_left_first(Transform_T transform, Operation_T operation)
{
    return apply<IndexSequence_T>([&transform, &operation]<std::size_t... indices_T>() {
        return [&transform, &operation]<std::size_t index_T, typename Accumulated_T>(
                   this auto&& self, Accumulated_T&& accumulated
               ) {
            if constexpr (index_T == sizeof...(indices_T)) {
                return std::forward<Accumulated_T>(accumulated);
            }
            else {
                return self.template operator()<index_T + 1>(std::invoke(
                    operation,
                    std::forward<Accumulated_T>(accumulated),
                    transform.template operator()<index_T>()
                ));
            }
        }.template operator()<1>(transform.template operator()<indices_T...[0]>());
    });
}

export template <type_list_c TypeList_T, typename Transform_T, typename Operation_T>
    requires(type_list_size_v<TypeList_T> != 0)
constexpr auto fold_left_first(Transform_T transform, Operation_T operation)
{
    return fold_left_first<std::make_index_sequence<type_list_size_v<TypeList_T>>>(
        [&transform]<std::size_t index_T> {
            return transform.template operator()<type_list_at_t<TypeList_T, index_T>>();
        },
        operation
    );
}

}   // namespace ddge::util::meta
