...
module ddge.scheduler.data_structures.WorkTree;

import ddge.scheduler.data_structures.SignalTree;
import ddge.scheduler.data_structures.Work;
...
auto ddge::scheduler::WorkContract::schedule() -> bool
{
...
auto ddge::scheduler::WorkContract::schedule_release() -> WorkContinuation
{
    if (!m_release.has_value()) {
...
auto ddge::scheduler::WorkContract::release() -> void
{
    if (m_release.has_value()) {
        m_release->operator()();
    }
...
) -> uint64_t
{
    return std::max(
        ...
        static_cast<uint64_t>(ddge::scheduler::SignalTree::minimum_number_of_levels())
    );
}
...
) -> uint32_t
{
    ...
    return std::max(
        ...
        ddge::scheduler::SignalTree::minimum_number_of_levels()
    );
}

ddge::scheduler::WorkTree::WorkTree(
...
auto ddge::scheduler::WorkTree::try_emplace(Work&& work) -> std::expected<WorkIndex, Work>
{
...
auto ddge::scheduler::WorkTree::try_emplace(Work&& work, ReleaseWorkContract&& release)
...
auto ddge::scheduler::WorkTree::try_emplace_at(const WorkIndex work_index, Work&& work)
...
auto ddge::scheduler::WorkTree::try_emplace_at(
...
[[nodiscard]]
constexpr auto default_strategy(uint32_t) -> ddge::scheduler::TravelsalBias
{
    return ddge::scheduler::TravelsalBias::eLeft;
}

auto ddge::scheduler::WorkTree::try_execute_one_work(const uint32_t thread_id) -> bool
{
    PRECOND(
...
auto ddge::scheduler::WorkTree::schedule(const WorkIndex work_index) -> void
{
...
auto ddge::scheduler::WorkTree::schedule_for_release(const WorkIndex work_index) -> void
{
...
auto ddge::scheduler::WorkTree::capacity() const noexcept -> uint64_t
{
    return m_work_contracts.size();
}

auto ddge::scheduler::WorkTree::optimized_for_thread_count() const noexcept -> uint32_t
{
    return static_cast<uint32_t>(m_contract_signals.size());
}

auto ddge::scheduler::WorkTree::try_emplace(
    Work&&                               work,
    std::optional<ReleaseWorkContract>&& release
...
auto ddge::scheduler::WorkTree::handle_work_result(
    const WorkIndex        work_index,
    const WorkContinuation work_continuation
) -> void
{
