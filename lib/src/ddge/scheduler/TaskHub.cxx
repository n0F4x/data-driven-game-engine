...
auto ddge::scheduler::TaskHub::set_indirect_tasks(std::vector<Task>&& indirect_tasks)
    -> void
{
    m_indirect_tasks = std::move(indirect_tasks);
}

auto ddge::scheduler::TaskHub::try_emplace_generic_at(
    Task&&          task,
...
auto ddge::scheduler::TaskHub::try_emplace_main_only_at(
    Task&&          task,
    const TaskIndex task_index
) -> std::expected<void, Task>
{
...
auto ddge::scheduler::TaskHub::schedule(const TaskIndex task_index) -> void
{
...
auto ddge::scheduler::TaskHub::try_execute_a_generic_task(const uint32_t thread_id)
    -> bool
{
    return m_generic_work_tree.try_execute_one_work(thread_id);
}

auto ddge::scheduler::TaskHub::try_execute_a_main_only_task() -> bool
{
    return m_main_only_work_tree.try_execute_one_work(0);
}

auto ddge::scheduler::TaskHub::try_emplace_embedded_task_at(
    Task&&          task,
    const TaskIndex task_index
) -> std::expected<void, Task>
{
...
auto ddge::scheduler::TaskHub::select_work_tree(const TaskIndex task_index) -> WorkTree&
{
