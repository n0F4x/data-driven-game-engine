...
import ddge.deprecated.gltf.Mesh;
...
public:
    ...
    [[nodiscard]]
    auto mesh(const Model& model) const -> util::OptionalRef<const Mesh>;
...
public:
    ...
    [[nodiscard]]
    static auto
        load_from_file(const std::filesystem::path& filepath, std::size_t scene_index)
            -> std::optional<Model>;
