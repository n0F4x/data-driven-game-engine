...
[%collapsible]
====
Run the following command: `+conan export-pkg .+` to export the library to the local conan cache.
Test the exported library using `+conan test data-driven_game_engine/version --profile:host=customtestprofile --profile:build=customtestprofile+`.
Make sure that `+user.data-driven_game_engine:dev+` is not declared as `+True+` in the conan profile used for testing.
