...
[,c++]
."Hello World!" using the execution plugin
----
ddge::app::create()
    .plug_in(ddge::plugins::Execution{})
    .run(ddge::exec::as_task([] { std::println("Hello World!"); }));
----

We can also describe more complicated logic using execution primitives.
...
ddge::app::create()
    .plug_in(ddge::plugins::Execution{})
    .run(
        ddge::exec::start_as(ddge::exec::as_task(say_hello))
            .then(ddge::exec::as_task(say_hi))
    );
----
...
----
namespace ex = ddge::exec;

auto schedule =
    ex::start_as(initialize_app)
        .then(
            ex::loop_until(
                ex::group(
                    ex::as_task(process_events),
                    ex::as_task(update),
                    ex::as_task(render)
                ),
                ex::as_task(game_is_running)
            )
        )
        .then(shut_down);
----
