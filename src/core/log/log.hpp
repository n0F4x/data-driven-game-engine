...
#define ENGINE_LOG_LEVEL_TRACE    6
  ...
  #define ENGINE_LOG_ERROR(...) \
      SPDLOG_LOGGER_CALL(&core::log::internal::logger(), spdlog::level::err, __VA_ARGS__)
#else
  ...
  #define ENGINE_LOG_DEBUG(...)                                             \
      SPDLOG_LOGGER_CALL(                                                   \
          &core::log::internal::logger(), spdlog::level::debug, __VA_ARGS__ \
      )
#else
  ...
  #define ENGINE_LOG_TRACE(...)                                             \
      SPDLOG_LOGGER_CALL(                                                   \
          &core::log::internal::logger(), spdlog::level::trace, __VA_ARGS__ \
      )
#else
