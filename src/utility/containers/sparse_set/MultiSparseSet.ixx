...
class MultiSparseSet<Key_T, TypeList_T<Ts...>, version_bits_T>
    : SparseSetBase<Key_T, version_bits_T> {
public:
    using Key = Key_T;
...
template <
    ...
    requires(sizeof(Key_T) * 8 > version_bits_T)
constexpr auto
    util::sparse_set::MultiSparseSet<Key_T, TypeList_T<Ts...>, version_bits_T>::erase(
        const Key key
...
template <
    ...
    requires(sizeof(Key_T) * 8 > version_bits_T)
template <typename Self_T>
constexpr auto util::sparse_set::MultiSparseSet<Key_T, TypeList_T<Ts...>, version_bits_T>::
    get(this Self_T&& self, const Key key)
        -> std::tuple<meta::forward_like_t<Ts, Self_T>...>
{
    const Index id{ id_from_key(key) };
...
template <
    ...
    requires(sizeof(Key_T) * 8 > version_bits_T)
constexpr auto
    util::sparse_set::MultiSparseSet<Key_T, TypeList_T<Ts...>, version_bits_T>::find(
        const Key key
    ) -> std::optional<std::tuple<Ts&...>>
{
    const Index id{ id_from_key(key) };
...
static_assert(
    [] {
        util::sparse_set::MultiSparseSet<Key, Values> multi_sparse_set;
