...
auto initialize_map(
    ...
    const resources::Resource<const game::Settings> settings,
...
) -> void
{
    ...
    for (const uint8_t row_index : std::views::iota(uint8_t{}, settings->cells_per_row)) {
             ...
             std::views::iota(uint8_t{}, settings->cells_per_column))
        {
            registry->create(
                game::Cell{
                    .position = game::Position{ .row = row_index, .column = column_index },
                    .shape = make_shape(
...
) -> void
{
    ...
    registry->insert(
        *snake_head_id,
        game::Snake{ .charge = 1 },
        game::SnakeHead{ .direction = snake_direction }
    );
}
...
) -> void
{
    ...
    if (game_state.has_value()) {
        ...
        game_state.emplace(game::GameState{ .textures = { std::move(apple_texture) } });
    }
}
...
auto game::tasks::initialize() -> ddge::scheduler::TaskBuilder<void>
{
